...
3. 전체 대기 인원은 14,000,605명입니다. 우리는 14,000,606번째와 14,000,607번째에 배를 타게 됩니다. 
앞사람이 아프거나, 대기를 못하고 빠질 경우 대기인원이 줄어들 수도 있습니다. 
**라이캣과 자바독이 다른 배를 타야 할 경우에는 뒷배를 타야 합니다.**

4. 1월은 1024일, 2월은 512일, 3월은 256일, 4월은 128일, 5월은 64일, 6월은 32일, 
...
# 10분 마다 15명씩 + 정시에는 25명 => 1시간에 100명 씩 12시간 => 1200명
...
def solution(대기인원):
    ...
    해당시간에남은인원 = 최종남은인원 % 100  # 99 + 1의 경우에는 시간으로 넘어가 +1시간을 해주어야 함
    for i in 출발분:
        if i > 해당시간에남은인원:
            분 = 출발분.index(i) * 10
            break
    ...
    print( f'{년+2020}년 {월}월 {일}일 {str(시).zfill(2)}시 {str(분).zfill(2)}분 출발')
